...
class NeuralNetwork {
  constructor(arr, lr) {
    ...
    for (let i = 0; i < this.nodes.length - 1; i++) {
      ...
      this.values.push(new Matrix(this.nodes[i], 1).randomize())
    }
    for (let i = 1; i < this.nodes.length; i++) {
      this.biases.push(new Matrix(this.nodes[i], 1).randomize())
    }
    this.values.push(new Matrix(this.nodes[this.layers-1], 1).randomize())
  }
  static tanh(x) {
    var y = Math.tanh(x);
    return y;
...
function cross_over(NN1, NN2){
  let NN = new NeuralNetwork(NN1.nodes, NN1.lr)
  for(let i=0; i<NN1.weights.length; i++){
    for (let j = 0; j < NN1.weights[i].rows; j++) {
      for (let k = 0; k < NN1.weights[i].cols; k++) {
        let r = Math.random()
        if(r<0.5){
          NN.weights[i].data[j][k] = NN1.weights[i].data[j][k]
        }else{
          NN.weights[i].data[j][k] = NN2.weights[i].data[j][k]
        }
      }
    }
  }
  for (let i = 0; i < NN1.biases.length; i++) {
    for (let j = 0; j < NN1.biases[i].rows; j++) {
      for (let k = 0; k < NN1.biases[i].cols; k++) {
        let r = Math.random()
        if(r<0.5){
          NN.biases[i].data[j][k] = NN1.biases[i].data[j][k]
        }else{
          NN.biases[i].data[j][k] = NN2.biases[i].data[j][k]
        }
      }
    }
  }
